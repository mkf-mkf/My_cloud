...
def make_main_chart(chart_df, drawing_trades, spy_chart):
   ...
   Gross: {drawing_trades.Gross.sum()}   Vol: {int(drawing_trades.Qty.apply(abs).sum())}   Net: {drawing_trades.Net.sum()}',
    ...
    plot(dict(data=data, layout=layout), filename=f'{buy_executions.Date_Time.iloc[0].strftime("%Y-%m-%d")}_{buy_executions.Ticker[0]}.html')
